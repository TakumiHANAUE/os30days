...
void sheet_updown(struct SHEET *sht, int height)
{
    ...
    if (old > height) /* 以前よりも低くなる */
    {
        if (height >= 0) /* 間のものを引き上げる */
        {
            ...
            sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, height + 1); /* 新しい下じきの情報に沿って画面を描き直す */
        }
        else /* 非表示化 */
        {
            if (ctl->top > old)
            ...
            ctl->top--; /* 表示中の下じきが1つ減るので、一番上の高さが減る */
            sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, 0); /* 新しい下じきの情報に沿って画面を描き直す */
        }
    }
